...
**Goal:** Add a GitHub Actions workflow that builds `pgq` for macOS, Linux, and Windows on x86_64 and arm64, publishes tagged releases to GitHub Releases, and keeps release packaging consistent with the local `dist/pgq` contract.
...
- [ ] Step 3: Implement the minimum scripts/workflow structure needed for those tests
...
- [ ] Step 1: Add a target-aware packaging script that assembles `bin/pgq(.exe)`, `README.md`, and the skill under a versioned package root
